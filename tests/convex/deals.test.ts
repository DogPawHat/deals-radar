...
describe("deals", () => {
  describe("getDealsForStore", () => {
    it("returns empty array when no deals exist", async () => {
      ...
      const deals = await t.query(refs.public.deals.getDealsForStore, { storeId });
    ...
    it("returns deals for specific store", async () => {
      ...
      const dealsStore1 = await t.query(refs.public.deals.getDealsForStore, {
        storeId: storeId1,
      });
      const dealsStore2 = await t.query(refs.public.deals.getDealsForStore, {
        storeId: storeId2,
      });
